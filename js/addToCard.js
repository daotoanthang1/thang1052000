...
//             alert("This product is aready added to cart");
//             return;
//         }
// }
//     var cartRowContent = `
...
document.addEventListener("DOMContentLoaded", function () {
    var buttonAddItem = document.getElementsByClassName("add-to-card");
    for (let i = 0; i < buttonAddItem.length; i++) {
        buttonAddItem[i].addEventListener("click",cartItem)
    }
})        
    function cartItem(event) {
        var product = event.target;
        console.log(product);
        var products = product.parentElement.parentElement.parentElement;
        var imgSrc = products.getElementsByClassName("product-img")[0].src;
        var titleProduct = products.getElementsByClassName("title")[0].innerText;
        var priceProduct = products.getElementsByClassName("productItemPriceNew")[0].innerText;
        var quatityProduct = document.getElementById('my-badge');
        var cartItem = document.querySelector('.cart-item');
        var cartRow = document.createElement('div');
        var cartItemNames = document.getElementsByClassName("cart-item-title")
        cartRow.classList.add("cart-row");
        for(let i=0;i<cartItemNames.length;i++){
            if(cartItemNames[i].innerText == titleProduct){
                alert("This item is already add to cart")
                return;
            }
        }
        quatityProduct.textContent++;
        var cartRowContent = `
        <div class="cart-item-info">
            <img class="cart-item-image" src="${imgSrc}" alt="">
            <p class="cart-item-title">${titleProduct}</p>
            <p class="cart-price">${priceProduct}</p>
        </div>
